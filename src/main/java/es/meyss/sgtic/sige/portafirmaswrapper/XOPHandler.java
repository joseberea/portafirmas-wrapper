package es.meyss.sgtic.sige.portafirmaswrapper;
...
public class XOPHandler extends BasicHandler {
	...
	private static Logger logger = Logger.getLogger("XOPHandler");
    ...
    public void invoke(MessageContext msgContext) throws AxisFault {
    	...
    	if (isTargetedServiceAndOperation(msgContext)) {
			...
			if (mtomNodePath == null) {
				logger.info("No se ha especificado el parametro mtomNodePath en el fichero client-config.wsdd");
			} else {
				...
				try {
		            ...
		            if (currentNode != null) {
		            	...
		            	if (currentNode.getChildNodes() != null && 
		            		...
		            		logger.info("No se ha encontrado la referencia en el nodo " + mtomNodePath);
		            	}
		            	...
		            	if (attachmentRef != null) {
		            		...
		            		logger.info("No se ha encontrado la referencia al fichero XOP en el nodo" + mtomNodePath);
		            	}
			            
		            } else {
		            	
		            	logger.info("Nodo " + mtomNodePath + " no encontrado en el SOAP Body");
    ...
    private boolean isTargetedServiceAndOperation(MessageContext msgContext) {
        ...
        if (serviceName == null) {
        	logger.info("No se ha especificado la propiedad serviceName en el fichero client-config.wsdd");
        }
        
		Service locator = (Service) msgContext.getProperty(Call.WSDL_SERVICE);
		if (locator != null && locator.getServiceName() != null && locator.getServiceName().toString().equals(serviceName)) {
	    	String operationName = (String) getOption(HANDLER_OPTION_OPERATION_NAME);
	        if (operationName == null) {
	        	logger.info("No se ha especificado la operacion operationName en el fichero client-config.wsdd");
